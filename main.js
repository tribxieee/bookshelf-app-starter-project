...
function createBookElement(book) {
  ...
  const statusBtnLabel = book.isComplete ? "Selesai dibaca" : "Tandai Selesai";
  ...
  wrapper.innerHTML = `
    ...
    <div class="d-flex justify-content-between mt-2 gap-2">
      <button class="btn btn-sm ${statusBtnClass} flex-grow-1" data-testid="bookItemIsCompleteButton">
        ${statusBtnLabel}
      </button>
      <button class="btn btn-warning btn-sm text-white" data-testid="bookItemEditButton">Edit</button>
      <button class="btn btn-danger btn-sm" data-testid="bookItemDeleteButton">Hapus</button>
    </div>
  `;
  col.appendChild(wrapper);

  wrapper.querySelector('[data-testid="bookItemIsCompleteButton"]').addEventListener("click", () => toggleBookStatusWithConfirm(book.id));
  wrapper.querySelector('[data-testid="bookItemEditButton"]').addEventListener("click", () => enterEditMode(book.id));
  wrapper.querySelector('[data-testid="bookItemDeleteButton"]').addEventListener("click", () => deleteBook(book.id));
...
function renderBooks() {
  ...
  books.forEach(book => {
    ...
    if (book.isComplete) completeList.appendChild(elm);
    else incompleteList.appendChild(elm);
  });
...
form.addEventListener("submit", e => {
  ...
  if (editingId) {
    ...
    const newBook = { id: generateId(), title, author, year, isComplete };
    books.push(newBook);
    saveData();
    renderBooks();
...
/* Search submit */
searchForm.addEventListener("submit", e => { e.preventDefault(); renderBooks(); });
...
function deleteBook(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      if (editingId === id) { editingId = null; form.reset(); submitButton.innerHTML = `<i class="bi bi-plus-circle"></i> Simpan Buku`; }
      saveData();
      renderBooks();
...
function enterEditMode(id) {
  ...
  Swal.fire({ icon: "info", title: "Edit mode aktif", text: "Ubah data lalu tekan Perbarui Buku", timer: 1100, showConfirmButton: false });
  titleInput.focus();
}
...
/* Filter buttons */
[btnAll, btnUnread, btnRead].forEach(btn => btn.addEventListener("click", () => {
  if (btn.id === "filter-all") currentFilter = "all";
  else if (btn.id === "filter-unread") currentFilter = "unread";
  else if (btn.id === "filter-read") currentFilter = "read";
  saveFilter();
  setActiveFilterButton(currentFilter);
  renderBooks();
}));
